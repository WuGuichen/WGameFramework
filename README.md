...
- `Assets/UI/**/*.uxml` and `Assets/UI/**/*.uss` UI Toolkit text sources plus their `.meta` files
...
- Unity project settings, scenes, art assets, non-text UI assets, runtime configs, FMOD/third-party plugins, and package cache
...
- UI Toolkit `.uxml` and `.uss` files are included because they are reviewable text and explain demo/control surfaces.
- Demo scenes, generated materials, PanelSettings assets, FMOD plugin files, banks, project settings, and other Unity assets stay in the SVN project only.
